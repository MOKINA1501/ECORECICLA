...
class DatamaxModel extends Model
{
    ...
    **/
    ...
    public function get_menu()
    {
        ...
        $builder                                                                =   $this->db->table('dtm_submodules')
            ->select('dtm_submodules.name')
            ->distinct('dtm_submodules.name')
            ->join('dtm_modules', 'dtm_modules.id = dtm_submodules.module')
            ->join('dtm_permissions', 'dtm_permissions.submodule = dtm_submodules.id')
            ->where('dtm_submodules.flag_drop', 0)
            ->where('dtm_modules.menu', 1)
            ->where('dtm_permissions.role', $this->session->get('data.role'))
            ->get();
            
        $result['menu_right']                                                   =   count($builder->getResultArray());
    ...
    **/
    ...
    public function select_data($fields, $drop, $table, $order, $dir)
    {
        ...
        if ($prefix[0] == 'git')
        {
            $this->builder->where('git_company != ', 'G');
        }

        $this->builder->order_by($order, $dir);
        $query                                                                  =   $this->builder->get($table);
    ...
    **/
    ...
    public function select_single_data(string $fields, bool $drop, string $where_field, string|int $where_value, string $table)
    {
    ...
    **/
    ...
    {
        $builder                                                                =   $db->table($table);
        $result                                                                 =   $this->builder->insert($table, $params);

        if ($result)
        {
            if ($this->builder->insert_id() == 0)
            {
                return true;
            }
            else
    ...
    **/
    public function update_data($params, $name_id, $table)
    {
        ...
        $id                                                                     =   $params['id'];
        unset($params['id']); 
        $this->builder->where($name_id, $id);
        return $this->builder->update($table, $params);
        exit();
    }
    ...
    **/
    public function udrop_data($name_id, $value_id, $table, $params)
    {
        ...
        $this->builder->where($name_id, $value_id);
    ...
    **/
    ...
    {
        $builder                                                                =   $db->table($table);
        return $this->builder->delete($table, $data);
        exit();
    }
    ...
    **/
    public function send_mail($address, $subject, $content)
    {
        ...
        //$email->from('datamax@gmail.com', $content['of']);
       $email->setFrom('gestion@datamax.co', $content['of']);
       $email->setTo($address);
       ...
       $email->set_newline("\r\n");
        ...
        if (!$email->send())
        {
            return show_error($email->print_debugger());
        }
        else
        {
            return TRUE;
    ...
    **/
    public function actions_by_role($role, $submodule)
    {
        ...
        // {
        //     $this->builder                                                      =   $this->db->table('dtm_users_hcal');
        //     $this->builder->select('role');
        //     $this->builder->where('id', 1);

        //     $query_roles                                                        =    $this->builder->get();
        ...
        $this->builder->select('name_action');
        $this->builder->wherein('role', $roles_user);
        $this->builder->where('name_submodule', $submodule);
    ...
    //     {
    //         $this->builder->select('id_role');
    //         $this->builder->where('id_user', $this->session->userdata['id_user']);

    //         $query_roles                                                        =   $this->builder->get('fet_users_roles');
    ...
    //     $this->builder->select('name_action');
    //     $this->builder->where_in('id_role', $roles_user);
    //     $this->builder->where('name_submodule', $submodule);
    //     $this->builder->where('git_company != ', 'G');

    //     $query                                                                  =   $this->builder->get('git_permissions');
    ...
    **/
    ...
    {
        $builder                                                                =   $db->table('dtm_submodules');
        $this->builder->select('dtm_modules.name_es, dtm_modules.name, dtm_submodules.name_es, dtm_submodules.url');
        $this->builder->join('dtm_modules', 'dtm_modules.id = dtm_submodules.module');
        $this->builder->where('dtm_submodules.name', $submodule);
        // $this->builder->where('git_submodules.git_company != ', 'G');

        $query                                                                  =   $this->builder->get('dtm_submodules');
    ...
    // public function get_breadcrumb($submodule)
    // {
    //     $this->builder->select('git_modules.name_es_module, git_modules.name_module, git_submodules.name_es_submodule, git_submodules.url_submodule');
    //     $this->builder->join('git_modules', 'git_modules.id_module = git_submodules.id_module');
    //     $this->builder->where('git_submodules.name_submodule', $submodule);
    //     $this->builder->where('git_submodules.git_company != ', 'G');

    //     $query                                                                  =   $this->builder->get('git_submodules');
    ...
    **/
    ...
    //     $this->builder->select('gc.id_company AS id, gc.code_company AS text');
    //     $this->builder->join('git_projects gp', 'gp.id_company = gc.id_company');
    //     $this->builder->join('git_users_projects gu', 'gu.id_project = gp.id_project');
    //     $this->builder->where('gc.git_company != ', 'G');
    //     $this->builder->where('gc.flag_association_active', 1);
    //     $this->builder->where('gc.flag_internal', 1);
    //     $this->builder->where('gc.flag_drop', 0);
    //     $this->builder->where('gu.id_user', $this->session->userdata['id_user']);

    //     if (isset($params['q']) && $params['q'] != '')
    //     {
    //         $this->builder->like('gc.code_company', $params['q']);
    //     }

    //     $this->builder->group_by('gc.id_company, gc.code_company');
    //     $this->builder->order_by('gc.code_company', 'asc');
    //     $this->builder->limit($limit, $start);

    //     $query                                                                  =   $this->builder->get('git_companies gc');
    ...
    **/
    ...
    //         $this->builder->select('gp.id_project AS id, CONCAT(gp.name_project, " - ", gp.neighborhood_project) AS text');
    //         $this->builder->join('git_users_projects gu', 'gu.id_project = gp.id_project');
    //         $this->builder->where('gp.git_company != ', 'G');
    //         $this->builder->where('gp.id_company', $params['id']);
    //         $this->builder->where('gp.flag_drop', 0);
    //         $this->builder->where('gu.id_user', $this->session->userdata['id_user']);

    //         if (isset($params['q']) && $params['q'] != '')
    //         {
    //             $this->builder->group_start();
    //             $this->builder->like('gp.name_project', $params['q']);
    //             $this->builder->or_like('gp.neighborhood_project', $params['q']);
    //             $this->builder->group_end();
    //         }

    //         $this->builder->group_by('id, text');
    //         $this->builder->order_by('gp.name_project', 'asc');
    //         $this->builder->limit($limit, $start);

    //         $query                                                              =   $this->builder->get('git_projects gp');
    ...
    **/
    ...
    //     if ($id != '')
    //     {
    //         $this->builder->select('gc.id_company, gc.code_company, CONCAT(gp.name_project, " - ", gp.neighborhood_project) AS project');
    //         $this->builder->join('git_companies gc', 'gc.id_company = gp.id_company');
    //         $this->builder->where('gp.id_project', $id);
    ...
    **/
    ...
    //     if ($id) 
    //     {
    //         $this->builder->select('img_logo_company');
    //         $this->builder->where('id_company', $id);

    //         $query                                                              =   $this->builder->get('git_companies');
    ...
    **/
    ...
    {
        $this->builder                                                          =   $this->db->table($table . 'A');
        $this->builder->select('IFNULL(AI.name_area, "NO APLICA") AS area_insert');
        $this->builder->select('IFNULL(CONCAT(I.name_user, " ", I.lastname_user), "NO APLICA") AS user_insert');
        $this->builder->select('IFNULL(DATE_FORMAT(A.date_insert, "%d-%m-%Y"), "NO APLICA") AS date_insert');
        $this->builder->select('IFNULL(DATE_FORMAT(A.date_insert, "%h:%i %p"), "NO APLICA") AS hour_insert');

        $this->builder->select('AU.name_area AS area_update');
        $this->builder->select('CONCAT(U.name_user, " ", U.lastname_user) AS user_update');
        $this->builder->select('DATE_FORMAT(A.date_update, "%d-%m-%Y") AS date_update');
        $this->builder->select('DATE_FORMAT(A.date_update, "%h:%i %p") AS hour_update');

        $this->builder->join('git_users I', 'I.id_user = A.user_insert', 'left');
        $this->builder->join('git_areas AI', 'AI.id_area = I.id_area', 'left');

        $this->builder->join('git_users U', 'U.id_user = A.user_update', 'left');
        $this->builder->join('git_areas AU', 'AU.id_area = U.id_area', 'left');

        $this->builder->where('A.' . $id_table . ' =', $id);
    ...
    **/
    ...
    //     $this->builder->select('IFNULL(A.action, \'NO APLICA\') AS module_action');
    //     $this->builder->select('IFNULL(DATE_FORMAT(A.date_update, "%d-%m-%Y"), \'NO APLICA\') AS date_update');
    //     $this->builder->select('IFNULL(LOWER(DATE_FORMAT(A.date_update, "%h:%i %p")), \'NO APLICA\') AS hour_update');
    //     $this->builder->select('IFNULL((SELECT CONCAT(C.name, " ", C.lastname) FROM dtm_users C WHERE C.id = A.user_update), \'NO APLICA\') AS user_update');
    //     $this->builder->select('IFNULL((SELECT C.document_number FROM dtm_users C WHERE C.id = A.user_update), \'NO APLICA\') AS document_number');
    //     $this->builder->select(' IFNULL((SELECT R.name FROM dtm_occupations R JOIN dtm_users C WHERE R.id = C.role limit 1), \'NO APLICA\') AS user_ocupation');

    //     $this->builder->join('dtm_users I', 'I.id = A.user_update', 'left');
    //     $this->builder->join('dtm_occupations AI', 'AI.id = I.id', 'left');

    //     $this->builder->join('dtm_users U', 'U.id = A.user_update', 'left');
    //     $this->builder->join('dtm_occupations AU', 'AU.id = U.id', 'left');

    //     $this->builder->where('A.'. $id_table .'=',$id);
    //     $this->builder->orderBy('UNIX_TIMESTAMP(date_update) DESC');
    ...
    //     // if(count($query->getResult('array')) == 1) $class = 'unico';
    //     die(var_dump($query));
    //     if (count($query->getResult('array')) > 0)
    //     {
    //         // if (boolval($this->session->userdata['mobile'])) 
    ...
    //         //                                                                             <div class="tl-content">
    //         //                                                                                 <div class="tl-subcontent media align-items-center wd-100p-force mg-y-10-force">
    //         //                                                                                     <div class="profile_letter_mobile">' . mb_strtoupper(substr($data['user_update'], 0, 1)) . '</div> 
    //         //                                                                                     <div class="media-body">
    //         //                                                                                         <p class="tx-inverse tx-medium mg-b-0 tx-12">' . mb_strtoupper($data['user_update']) . '</p>
    //         //                                                                                         <p class="tx-10 mg-b-0">
    //         //                                                                                             <span>' . $data['user_ocupation'] . '</span><br>
    //         //                                                                                             <span>' . $data['document_number'] . '</span><br>
    //         //                                                                                             <span class="mr-1"><b>Fecha: </b>' . $data['date_update'] . '</span>
    //         //                                                                                             <span><b>Hora: </b>' . $data['hour_update'] . '</span><br>';

    //         //                                                                                             switch ($data['action']) 
    //         //                                                                                             {
    //         //                                                                                                 case 'ADD':
    //         //                                                                                                     $result['history'] .= '<span><b>Acción:</b> Crear ' . $module . '</span>';
    //         //                                                                                                     break;
    //         //                                                                                                 case 'EDIT':
    //         //                                                                                                     $result['history'] .= '<span><b>Acción:</b> Editar ' . $module . '</span>';
    //         //                                                                                                     break;
    //         //                                                                                                 case 'UDROP':
    //         //                                                                                                     $result['history'] .= '<span><b>Acción:</b> Eliminar ' . $module . '</span>';
    //         //                                                                                                     break;                                                                                                            
    //         //                                                                                                 default:
    //         //                                                                                                     $result['history'] .= '<span><b>Acción:</b> No aplica</span>';
    //         //                                                                                                     break;
    //         //                                                                                             }
    ...
    //         //                                                                                     </div>
    //         //                                                                                 </div>';
    //         //         if ($key === array_key_first($query->result_array())) 
    //         //         {
    //         //             $result['history']                                      .=          '<div class="pd-x-20 pd-y-5 lasted-trace">
    //         //                                                                                     <p class="tx-12 tx-spacing-1 mg-0 pd-0">
    //         //                                                                                         <span class="tx-12 tx-roboto">Última edición</span>
    //         //                                                                                     </p>
    //         //                                                                                 </div>';
    ...
    //         // else
    //         // {
    //             foreach ($query->getResult('array') as $key => $data)
    //             {
    //                 $result['history']                                          .= '<div class="tl-item">
    ...
    //                                                                                                     <p class="text-cargo">' . ucfirst(mb_strtolower($this->model->accents($data['user_ocupation']))) . '</p>
    //                                                                                                 </div>
    //                                                                                             </div>
    ...
    public function global_trace_register_new($table, $id_table, $id, $module)
    {
        ...
        $this->builder->select('IFNULL(AI.name, "NO APLICA") AS cargo_update, A.action AS module_action');
        $this->builder->select('CONCAT(I.name, " ", I.lastname) AS user_insert, CONCAT(I.document_type, " ", I.document_number) AS document');

        $this->builder->select('AU.name AS cargo_update');
        $this->builder->select('CONCAT(U.name, " ", U.lastname) AS user_update');
        $this->builder->select('DATE_FORMAT(A.date_update, "%d-%m-%Y") AS date_update');
        $this->builder->select('DATE_FORMAT(A.date_update, "%h:%i %p") AS hour_update');
        ...
        $this->builder->orderby('UNIX_TIMESTAMP(A.date_update) DESC','DES');
        ...
        if (count($query->getResult('array')) > 0)
        {
        	$mobile                                                             =   $this->loginmodel->verify_device();
            if (boolval($mobile)) 
            {
                foreach ($query->getResult('array') as $key => $data)
                {
                    $result['history']                                          .= '<div class="tl-item">
                                                                                        ...
                                                                                        <div class="tl-content">
                                                                                            <div class="tl-subcontent media align-items-center wd-100p-force mg-y-10-force">
                                                                                                <div class="profile_letter_mobile">' . mb_strtoupper(substr($data['user_update'], 0, 1)) . '</div> 
                                                                                                <div class="media-body">
                                                                                                    <p class="tx-inverse tx-medium mg-b-0 tx-12">' . $data['user_update'] . '</p>
                                                                                                    <p class="tx-10 mg-b-0">
                                                                                                        <span>' . $data['area_update'] . '</span><br>
                                                                                                        <span class="mr-1"><b>Fecha: </b>' . $data['date_update'] . '</span>
                                                                                                        <span><b>Hora: </b>' . $data['hour_update'] . '</span><br>';
                    ...
                    if ($key === array_key_first($query->getResult('array'))) 
                    {
                        $result['history']                                      .=          '<div class="pd-x-20 pd-y-5 lasted-trace">
                                                                                                <p class="tx-12 tx-spacing-1 mg-0 pd-0">
                                                                                                    <span class="tx-12 tx-roboto">Última edición</span>
            ...
            else
            {
                ...
                {
                    $result['history']                                          .= '<div class="tl-item">
                                                                                        <div class="tl-dut"></div>
                                                                                        <div class="tl-content">
                                                                                            <div class="row mg-0 pd-x-30 pd-y-10 wd-100p">
                                                                                               <div class="row mg-0 col-lg-5 col-md-8 col-12 pd-0">
                                                                                                     <img src="' . site_url('users/photo/' . $data['user_update'] . '').'" class="photo-trace" >
                                                                                                    <div class="media-body mg-l-20 mg-l-xs-0">
                                                                                                        <p class="text-res">Responsable</p>
                                                                                                        <p class="text-name">' . ucwords(mb_strtoupper($data['user_update'])) . '</p>
                                                                                                        <p class="text-doc">' .  ucwords(mb_strtoupper($data['document'])) . '</p>
                                                                                                        <p class="text-cargo">' .  ucwords(mb_strtoupper($data['cargo_update'])) . '</p>
                                                                                                    </div>
                                                                                                </div><div class="tl-subcontent text-muted tx-16-force mt-4 col-lg-3 col-md-4 col-12 pd-0">
                                                                                                    <span class="text-tl">Acción</span>';

                                                                                                    switch ($data['module_action']) 
                                                                                                    {
                                                                                                        case 'ADD':
                                                                                                            $result['history'] .= '<span> Crear '. $module . '</span>';
                                                                                                            break;
                                                                                                        case 'EDIT':
                                                                                                            $result['history'] .= '<span> Editar ' . $module . '</span>';
                                                                                                            break;
                                                                                                        case 'UDROP':
                                                                                                            $result['history'] .= '<span> Eliminar ' . $module . '</span>';
                                                                                                            break; 

                                                                                                        default:
                                                                                                            $result['history'] .= ' No aplica';
                                                                                                            break;
                                                                                                    }
                                                                                                ...
                                                                                                <div class="tl-subcontent text-muted tx-16-force mt-4 col-lg-2 col-md-6 col-12 pd-0">
                                                                                                    <p class="text-tl">Fecha</p>
                                                                                                    <p class="text-sub">' . $data['date_update'] . '</p>
                                                                                                </div>
                                                                                                <div class="col-lg-2 col-md-6 col-12 mt-4 pd-0">
                                                                                                    <p class="text-tl">Hora</p>
                                                                                                    <p class="text-sub">' . $data['hour_update'] . '</p>
                                                                                                </div>
                                                                                                ';
